...
| **7** | 開いたエディタが入力を取りこぼす問題 | **完了** |

### フェーズ7: 開いたエディタが入力を取りこぼす

フェーズ6 でエディタが開くようになったあと、**Native Instruments の音源だけ
GUI を操作できない**という症状が出た。描画は正常で、FM8 では画面下の鍵盤だけが
反応する。

**原因は本体の再描画要求だった。プラグイン側でも VST3 側でもない。**

```rust
ctx.request_repaint_after(Duration::from_millis(16));   // ← これ
```

eframe は「期限を過ぎた再描画要求」を見つけると `ControlFlow::Poll` に落とす
（`native/run.rs` の `check_redraw_requests`）。vsync 60Hz のフレームは約 16.7ms
なので、**16ms を要求すると毎回すでに期限切れ**になり、ループが一度も待機状態に
入らなくなる。winit は `RedrawRequested` を配送するたびに配送ループを打ち切る作りなので
（`interrupt_msg_dispatch`）、待機に入らない限り**メッセージキューが常に捌き残る**。
割を食うのは同じスレッドに貼り付いたプラグインのエディタで、単発のクリックは通るのに
ホバーやドラッグが効かなくなる。

**16ms で再現、21ms で解消**（実測）。エディタを開いている間だけ 33ms にした。
開いていなければ誰も割を食わないので 16ms のまま。

#### 切り分けの経過（仮説を8つ潰した）

`editor_smoke` に `--hold` を足して**本体の外で触れる**ようにしたのが起点。
そこでは正常だったので、**素の Win32 ループと本体の差**を1つずつ埋めていった。

| 潰した仮説 | 潰し方 |
|---|---|
| 中身の大きさが窓と合っていない | 子ウィンドウの実寸を測った。一致 |
| 表示スケールによる座標ずれ | 100% だった |
| winit がメッセージを配送していない | `PeekMessageW(.., 0, 0, 0, PM_REMOVE)` で絞り込み無し |
| オーディオスレッドとの取り合い | `--busy` で並走させても正常 |
| 子ウィンドウが無効・位置ずれ | 列挙して確認。有効・表示・位置とも同一 |
| Qt 固有の問題 | FM8 の子は `NIVSTChildWindow` で Qt ではない |
| DPI 認識モードの食い違い | `--dpi-aware` / `--mixed-hosting` で変化なし |
| COM (STA) の初期化 | `OleInitialize` を足しても変化なし |
| vsync 待ちでスレッドが寝ている | `--no-vsync` で変化なし |

**`--egui` で再現できたのが決め手。** eframe の窓ごと並べた小さいプログラムを作り、
そこで症状が出た時点で、1600行の `main.rs` ではなくこの中で二分できるようになった。
「エディタを開く順番」「自前のメッセージ配送」「再描画要求」を1つずつ外して、
再描画要求で当たった。

**推測は8回とも外れた。当たったのは、再現する箱を作ってから中を削ったとき。**
フェーズ3 の「推測は2回とも外れた」と同じ轍で、症状から原因を当てにいくより、
**差を1つずつ埋めて再現させる**ほうが早い。

**途中で入れた対策のうち、効かなかったものは消した。** エディタの窓宛メッセージを
自前で捌く `pump_own_messages` は、この仮説が外れた時点で落としている
（winit のループの中から再入して配送する形なので、根拠を失ったまま残すのは危険）。

`editor_smoke --egui --repaint-ms N` は残してある。この壊れ方は自動では拾えないので、
再描画まわりを触ったら手で確かめること（16 で壊れ、21 以上で正常）。
