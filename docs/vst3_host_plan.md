...
| **4** | 仕上げ（CLAP と VST3 の混在確認、ドキュメント） | **完了**（テスト治具のみ保留） |
...
**テスト治具（clap-wrapper で test-plugin を VST3 化）は保留。** CMake が入っていない。
入れれば決定どおり進められるが、開発環境への追加インストールなので判断を仰ぐ。
今のところ VST3 側は実物（Surge XT）で検証している。治具があれば
「同じ DSP の CLAP 版と VST3 版で出力を突き合わせる」ができるようになる。
