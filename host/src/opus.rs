...
/// 測定と経緯は `docs/archive/export_rate_plan.md` の「高ビットレートは出さない」と
...
/// 符号化に使うスレッドのスタック。
///
/// **`OpusEncoder::new` が大量にスタックを使う。** 符号化ではなく生成の時点で
/// 落ちる。実測は `spike/opus/src/bin/stack_probe.rs` と `bin/opus_smoke.rs`。
///
/// | 版 | `size_of::<OpusEncoder>()` | release | debug |
/// |---|---|---|---|
/// | 0.1.26 | 1,288 バイト | 64KiB でも足りる | — |
/// | 0.1.27 / 0.1.28 | 254,608 バイト | 約 0.85MiB | 2MiB |
/// | 0.1.29 | 2,416 バイト | 約 0.38MiB | 約 0.53MiB |
///
/// 0.1.27 で符号化器の状態がヒープからインラインの固定長へ移ったのが発端。
/// **上流へは「ヒープに置く経路も欲しい」と出し、0.1.29 で入った**
/// (<https://github.com/restsend/opus-rs/issues/12>。既定で有効な `heap`
/// フィーチャ)。**書き出した `.opus` はバイト単位で 0.1.28 と一致する。**
///
/// # それでもこのスレッドは残す
///
/// 0.1.29 でも `CeltEncoder` の中身はインラインのままで、`new` は
/// `Box::new(CeltEncoder::new(..))` と書いている。**`Box::new` に渡す時点で
/// 一度スタックに載る**ので、2.4KB の構造体に対して 0.38MiB 要る。
///
/// **書き出しは `export_opus` からメインスレッド上で呼ばれ、Windows の
/// メインスレッドは既定で 1MiB しか無い。** 0.1.29 なら数字の上では収まるが、
/// **0.1.28 では本体の経路が release でも落ち、spike の同等コードは通った**
/// — 呼び出しの深さ次第で結果が変わる。**畳んでも得が無いので賭けない。**
///
