...
/// 選べるビットレート (kbps)。
///
/// **高いビットレートは出さない。** `opus-rs` 0.1.26 は、内容によって
/// **壊れたストリームを吐く** (本家 libopus で復号すると範囲外の値が散発し、
/// クリックノイズになる)。純音では 176kbps 以上で再現し、複雑な信号では
/// 192kbps でも出なかった。**信号依存なので、ビットレートだけでは安全な線を
/// 引けない。** 実測で問題の無かった範囲だけを出す。
/// 詳しくは `docs/export_rate_plan.md` の「高ビットレートは出さない」。
