...
- **音源とその音作りも保存される**。開き直すと同じ .clap が同じ状態で載る (下記)
...
- **パスだけでなくプラグイン ID も持つ。** 1つの `.clap` に複数のプラグインが
  入りうるので、パスだけでは特定できない
- **状態は `clap.state` 拡張で音源自身に書き出させる。** パラメータを列挙するのでは
  足りない (プリセットやウェーブテーブルなどパラメータに現れない状態があるため)。
  中身は不透明なバイト列で、ホストは解釈せず base64 で載せるだけ
...
- **音作りの復元は `clap.state` 拡張を持つ音源に限る**: 実装していない音源は、
