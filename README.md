...
- **打ち込みと音源の対応は多対多** — 1つの音源で複数の打ち込みを受けられる
  (ドラムをキックとハイハットで分けて書く) し、同じ打ち込みを複数の音源で重ねられる
- **プラグイン一覧** — フォルダを走査して音源/エフェクトに分け、お気に入りを星で残す
- **ピアノロールでの打ち込み** — 範囲選択・移動・伸縮・移調・連符、縦横のズーム
- **CC 段** — ペダルなどを「書いた区間だけ効く」形で書ける
- **ヴェロシティ段** — クレシェンド／デクレシェンドを、坂 (開始値→終了値) の
  ブロック1つで書ける。トラックの全ての段に効き、**音符のヴェロシティを直接書き換える**
...
(内訳は [docs/archive/vst3_host_plan.md](docs/archive/vst3_host_plan.md) のフェーズ6)。
...
| `test-plugin` | 検証用の CLAP プラグイン3つ (サイン波音源・ゲインエフェクト・出力を持たないモニタ)。**ホストとは何も共有せず、単体で他の DAW にも読ませられる** |
...
| | 内容 |
|---|---|
| [docs/guide.md](docs/guide.md) | **使い方・機能の詳細・アーキテクチャ・制限事項。現行はこちら** |
| [docs/archive/](docs/archive/) | 済んだ作業の**記録** (なぜそう作ったか・何を測ったか) |

`docs/archive/` にある文書は**ファイル名に `plan` と付いていますが、
やることリストではありません**。すべて完了済みの記録で、今のコードと
食い違っていることもあります。詳細は
[docs/archive/README.md](docs/archive/README.md) を参照してください。
